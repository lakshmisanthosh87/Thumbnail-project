...
const StyleSelector = ({value , onChange,isOpen , setIsOpen}:{value : ThumbnailStyle; onChange: (style: ThumbnailStyle) => void; isOpen: boolean; setIsOpen: (open: boolean) => void}) => {
    ...
    const styleDescriptions: Record<ThumbnailStyle, string> = {
        'Bold & Graphic': 'High constrasr,bold tyography,striking visuals.',
        'Minimalist': 'Clean, simple , lots of white space', 
        "photorealistic": "Photo-based, natural looking",
        "Illustrative": "Hand-drawn, artistic, creative",
  ...
  return (
    <div className='relative space-y-3 dark'>
        ...
        <button type='button' onClick={() => setIsOpen(!isOpen)} className='flex w-full items-center justify-between rounded-md border border-white/10 bg-white/8 px-4 py-3 text-left transition text-zinc-200 hover:bg-white/12'>
